...
### Phase 6: v2.0 — Architecture Refactoring & Reliability Hardening

A comprehensive refactoring pass addressing code structure, performance, reliability, and observability:

**Architecture (God Class → SRP)**

1. **UndoManager extraction** — Separated undo/redo stack management from `MainWindow` into a standalone 53-line module (`ui/undo_manager.py`). `MainWindow` now delegates via `self._undo_mgr`.

2. **ExportController extraction** — Extracted all export logic (JSON, CSV, COCO, VOC, YOLO, point cloud, logs) into `ui/export_controller.py` (160 lines). Eliminated dead `_ask_save_path` method from `MainWindow`.

3. **High-DPI adaptation** — Added `ctypes.windll.shcore.SetProcessDpiAwareness(1)` in `main.py` for crisp rendering on high-DPI Windows displays.

**Performance**

4. **O(1) incremental label counting** — Replaced `_rebuild_labeled_cache()` full-scan in `_update_status()` with `_sync_labeled_count_incremental()`. Tracks `_prev_image_name`, `_prev_ann_count`, and `_prev_image_index` to compute deltas in O(1).

5. **Pan optimization** — Replaced full `redraw()` in `on_pan_move` with `canvas.move("all", dx, dy)`, eliminating canvas item destruction/recreation during panning.

6. **Incremental selection redraw** — `select_annotation()` and `clear_selection()` now only redraw the old and new selection items instead of the entire canvas. Added `_undraw_annotation()` helper.

7. **Annotation list diff update** — `set_annotations_list()` in `Sidebar` now compares items one-by-one and only updates changed rows, reducing listbox flicker.

8. **Listbox color O(1) refresh** — `_refresh_listbox_color()` replaced with `_refresh_listbox_color_by_index()` that takes a pre-computed index instead of scanning by filename.

9. **Removed redundant redraw** — Eliminated duplicate `redraw()` calls in `on_release` after move/resize (already handled by `_update_dragged_annotation`).

10. **Eliminated getattr reflection** — `_prev_image_name` and `_prev_ann_count` initialized in `__init__` instead of using `getattr()` with defaults on every call.

**Reliability**

11. **Precise dirty data marking** — `_dirty = True` now set only on actual data changes (create/delete/move/resize/relabel). Removed unconditional `_dirty = True` from `_update_status()` that caused unnecessary auto-saves during browsing.

12. **Export validation** — Added `_validate_bbox()`, `_validate_annotation()`, and `_validate_project()` in `exporter.py`. `ExportController` runs validation before every export, catching empty labels, whitespace-only labels, out-of-range bbox coordinates, and zero-area boxes.

13. **Auto-save status feedback** — `except Exception: pass` replaced with visible status bar messages ("已自动保存" / "自动保存失败: {error}").

14. **Undo/redo explicit feedback** — Status bar now shows "已撤销 (剩余 X 步可撤销)" / "已重做 (剩余 X 步可重做)" / "无可撤销操作".

15. **Drag/resize syncs count** — `on_box_moved()` and `on_box_resized()` now call `_update_status()` to keep sidebar annotation list in sync.

**Observability**

16. **Confidence scoring for anomaly detection** — `anomaly_detect_annotations()` now returns `confidence` (low/medium/high) and `confidence_note`. Low confidence (< 3 samples) warns users in both verification summary and cross-image analysis.

17. **Cross-image confidence display** — `rgb_panel.py` shows "⚠样本不足" / "△样本偏少" markers next to labels with insufficient samples.

18. **Verification progress** — `verify_current_all()` shows "验证中: 3/12..." in the status bar when processing more than 5 annotations.

**Rendering**

19. **Adaptive image interpolation** — `Image.NEAREST` when `scale >= 2.0` (preserves pixel-art clarity at high zoom), `Image.BILINEAR` otherwise (smooth anti-aliased scaling).

20. **3D point cloud optimization** — Replaced random sampling with fixed-step sampling for reproducibility. Disabled `depthshade` for faster rendering.

**Testing**

21. **30 new test cases** — Added `TestUndoManager` (8 tests), `TestExportValidation` (12 tests), and `TestAnomalyConfidence` (4 tests). Total: 82 tests, all passing.
...
└── tests/
    └── test_core.py            # Unit tests (82 tests)
```
...
v2.0 是一次全面的架构重构与可靠性加固，涵盖代码结构、性能、可靠性和可观测性四个方面。

### 架构重构（God Class → 单一职责）

1. **UndoManager 拆分** — 将撤销/重做栈管理从 `MainWindow` 中抽出，形成独立的 53 行模块（`ui/undo_manager.py`），`MainWindow` 通过 `self._undo_mgr` 委托调用。

2. **ExportController 拆分** — 将所有导出逻辑（JSON、CSV、COCO、VOC、YOLO、点云、日志）抽取到 `ui/export_controller.py`（160 行），同时删除了 `MainWindow` 中废弃的 `_ask_save_path` 方法。

3. **高 DPI 适配** — 在 `main.py` 中添加 `ctypes.windll.shcore.SetProcessDpiAwareness(1)`，高分辨率屏幕下不再模糊。

### 性能优化

4. **标签计数 O(1) 增量更新** — `_update_status()` 中的 `_rebuild_labeled_cache()` 全量扫描替换为 `_sync_labeled_count_incremental()`，通过追踪 `_prev_image_name`、`_prev_ann_count`、`_prev_image_index` 计算差值，复杂度从 O(n) 降至 O(1)。

5. **平移优化** — `on_pan_move` 中的 `redraw()`（删除并重建所有 canvas 元素）替换为 `canvas.move("all", dx, dy)`，标注多时平移不再卡顿。

6. **选择标注增量重绘** — `select_annotation()` 和 `clear_selection()` 只重绘旧选中项（去除手柄）和新选中项（添加手柄），不再全量重建画布。

7. **标注列表差异更新** — `Sidebar.set_annotations_list()` 逐项比对，只更新变化的行，减少列表闪烁。

8. **列表颜色 O(1) 刷新** — `_refresh_listbox_color()` 线性扫描文件名找索引替换为 `_refresh_listbox_color_by_index()` 直接定位。

9. **移除冗余重绘** — `on_release` 中 move/resize 回调后删除多余的 `redraw()`（`_update_dragged_annotation` 已实时更新位置）。

10. **消除 getattr 反射** — `_prev_image_name` 和 `_prev_ann_count` 在 `__init__` 中初始化，每次 `_update_status` 不再做反射查询。

### 可靠性加固

11. **脏数据精确标记** — `_dirty = True` 仅在实际数据变更时设置（创建/删除/移动/缩放/改标签）。移除了 `_update_status()` 中无条件的 `_dirty = True`，浏览图片不再触发无意义的自动保存。

12. **导出校验** — 在 `exporter.py` 中新增 `_validate_bbox()`、`_validate_annotation()`、`_validate_project()`。`ExportController` 在每次导出前执行校验，拦截空标签、纯空格标签、超出 [0,1] 范围的 bbox 坐标和零面积框。

13. **自动保存状态反馈** — `except Exception: pass` 替换为状态栏可见提示（"已自动保存" / "自动保存失败: {错误信息}"）。

14. **撤销/重做明确反馈** — 状态栏显示 "已撤销 (剩余 X 步可撤销)" / "已重做 (剩余 X 步可重做)" / "无可撤销操作"。

15. **拖动/缩放同步计数** — `on_box_moved()` 和 `on_box_resized()` 末尾调用 `_update_status()`，侧边栏标注列表实时刷新。

### 可观测性

16. **异常检测置信度** — `anomaly_detect_annotations()` 返回新增 `confidence`（low/medium/high）和 `confidence_note` 字段。少于 3 个样本时标记为低置信度，在验证报告和跨图一致性分析中均会提醒用户。

17. **跨图一致性置信度展示** — `rgb_panel.py` 在低样本标签旁显示 "⚠样本不足" / "△样本偏少" 标记。

18. **验证进度提示** — `verify_current_all()` 标注数超过 5 个时，状态栏显示 "验证中: 3/12..."。

### 渲染优化

19. **自适应图片插值** — `scale >= 2.0` 时使用 `Image.NEAREST`（高倍放大保持像素清晰），否则使用 `Image.BILINEAR`（平滑抗锯齿）。

20. **3D 点云优化** — 随机采样替换为固定步长采样（结果可复现），关闭 `depthshade` 提升渲染速度。

### 测试

21. **新增 30 个测试用例** — 新增 `TestUndoManager`（8 个）、`TestExportValidation`（12 个）、`TestAnomalyConfidence`（4 个）。总计 82 个测试，全部通过。
